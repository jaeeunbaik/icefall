...
class PrototypeKMeansManager:
    ...
    def are_prototypes_initialized(self, layers: Optional[List[int]] = None) -> bool:
        """
        Check if prototypes are properly initialized for the specified layers.
        
        Args:
            layers: List of layer indices to check. If None, check all target_layers.
            
        Returns:
            True if all specified layers have initialized prototypes, False otherwise.
        """
        if layers is None:
        ...
        for layer_idx in layers:
            if layer_idx not in self.prototypes:
                return False
            if self.prototypes[layer_idx] is None:
                return False
            # Check if the prototype tensor is valid (not empty)
            if not isinstance(self.prototypes[layer_idx], torch.Tensor):
                return False
            if self.prototypes[layer_idx].numel() == 0:
                return False
    ...
    def sample_features_from_teacher(
        ...
        # Temporarily suppress stderr to avoid "forward pass called over 10 times" warnings
        # These warnings come from scaling modules during inference-only forward passes
        import sys
        import os
        ...
        with torch.no_grad():
            # Redirect stderr to devnull during feature sampling to suppress scaling warnings
            ...
            try:
                for batch_idx, batch in enumerate(dataloader):
                    ...
                    # Support several batch formats produced by our datasets/wrappers:
                    # - CleanNoisyWrapper: returns 'clean_audio' and 'audio'
                    # - HubertAsrDataset / older pipelines: 'inputs' or 'feature'
                    feature = None
                    supervisions = None
                    feature_lens = None

                    if 'clean_audio' in batch:
                        # Clean/noisy wrapper used for self-distillation
                        feature = batch['clean_audio'].to(self.device)
                        supervisions = batch.get('supervisions', None)
                    elif 'audio' in batch:
                        # Noisy primary audio (fallback to using this as input)
                        feature = batch['audio'].to(self.device)
                        supervisions = batch.get('supervisions', None)
                    elif 'inputs' in batch:
                        # Standard feature/audio naming used by some loaders
                        feature = batch['inputs'].to(self.device)
                        supervisions = batch.get('supervisions', None)
                    elif 'feature' in batch:
                        # Legacy feature format (already extracted)
                        feature = batch['feature'].to(self.device)
                        feature_lens = batch.get('feature_lens', None)
                        if feature_lens is not None and isinstance(feature_lens, torch.Tensor):
                    ...
                    # Prefer the explicit 'layer_results' if the model provides them
                    layer_output = None
                    try:
                        if isinstance(outputs, (list, tuple)) and len(outputs) >= 4 and outputs[3] is not None:
                            layer_results = outputs[3]
                            # If layer_idx not provided, use last available layer
                            idx = (layer_idx - 1) if layer_idx is not None else -1
                            # Guard index range
                            try:
                                layer_output = layer_results[idx]
                            except Exception:
                                layer_output = layer_results[-1]
                        else:
                            # Find first tensor-like object in outputs as a fallback
                        ...
                        logging.warning(f"Error extracting layer output: {e}")
                        continue

                    if layer_output is None:
                        logging.warning(f"Could not extract layer output from batch {batch_idx}; skipping")
                        continue
                    
                    # layer_output should be 3D: either (T, N, C) or (N, T, C)
                    if layer_output.dim() != 3:
                        logging.error(f"layer_output has wrong dimensions: {layer_output.shape}")
                        continue  # Skip this batch

                    # If first dim equals batch size, assume (N, T, C) and transpose
                    if layer_output.size(0) == feature.size(0):
    ...
    def initialize_prototypes(
        ...
        """
        logging.info(f"Initializing prototypes for layers {self.target_layers}")
        logging.info(f"  num_samples_per_layer: {num_samples_per_layer}")
        logging.info(f"  kmeans_iterations: {kmeans_iterations}")
        ...
        for layer_idx in self.target_layers:
            ...
            logging.info(f"Layer {layer_idx}: Starting feature sampling...")
            try:
                features = self.sample_features_from_teacher(
                    teacher_model, dataloader, num_samples_per_layer, layer_idx
                )
            except Exception as e:
                logging.error(f"Layer {layer_idx}: Feature sampling failed: {e}")
                raise
            
            # Apply PCA if requested
            if self.use_pca and features.shape[1] > self.pca_dim:
                logging.info(f"Layer {layer_idx}: Applying PCA {features.shape[1]} -> {self.pca_dim}")
                if not SKLEARN_AVAILABLE:
                    ...
                    if save_prototypes:
                        ...
                        with open(pca_file, 'wb') as f:
                            pickle.dump(pca, f)
                        logging.info(f"Layer {layer_idx}: PCA model saved")
            else:
                processed_features = features
            
            # Perform K-means clustering
            logging.info(f"Layer {layer_idx}: Starting K-means clustering...")
            try:
                ...
                logging.error(f"Layer {layer_idx}: K-means clustering failed: {e}")
                raise
                logging.error(f"[DEBUG] Layer {layer_idx}: Traceback:\n{traceback.format_exc()}")
                raise
            ...
            if save_prototypes:
                try:
                    ...
                    logging.info(f"Layer {layer_idx}: Saved prototypes to {prototype_file}")
                except Exception as e:
                    logging.error(f"Layer {layer_idx}: Failed to save prototypes: {e}")
                    raise
        
        logging.info(f"Prototype initialization completed for all layers")
    ...
    def compute_likelihood(
        ...
        if frame_mask is not None:
            # Handle potential shape mismatch between frame_mask and kl_per_frame
            if frame_mask.shape != kl_per_frame.shape:
                B_mask, T_mask = frame_mask.shape
                B_kl, T_kl = kl_per_frame.shape
                
                if B_mask != B_kl:
                    raise ValueError(f"Batch size mismatch: frame_mask {B_mask} vs kl_per_frame {B_kl}")
                
                # Adjust temporal dimension
                if T_mask > T_kl:
                    # Truncate mask to match kl_per_frame
                    frame_mask = frame_mask[:, :T_kl]
                elif T_mask < T_kl:
                    # Pad mask with False (invalid frames) to match kl_per_frame
                    padding = torch.zeros(B_mask, T_kl - T_mask, device=frame_mask.device, dtype=frame_mask.dtype)
