...
enable_rir=false              # RIR augmentation (DISABLED for self-distillation)
enable_cutmix=true 
...
resume_from=/home/hdd2/jenny/ASRToolkit/icefall/egs/librispeech/ASR/conformer_ctc/exp/models/best-valid-loss.pt

# Validation settings
enable_validation=true           # Re-enable after CleanNoisyWrapper fix
valid_interval=3000              # Keep larger interval for stability

# Learning Rate Scheduler Settings (Fine-tuning options)
scheduler_type="plateau"       # "noam", "plateau", "constant"
base_lr=2e-5                  # Increased for distillation training (was 5e-6)
scheduler_patience=3          # Patience for ReduceLROnPlateau
scheduler_factor=0.5          # Factor for ReduceLROnPlateau (0.5 = 50% reduction)
min_lr=5e-7                   # Increased minimum LR          
...
validation_skip_wer=false              # Enable WER computation now that decode_dataset is fixed

# Distillation Hyperparameters
enable_self_distillation=true
distill_layers=15                   # 0-based index, so 15 is the last layer for 16 total layers
distill_loss_type="mse"         # mse, cosine, kl
alpha=50                            # Increased from 10 to 50 for stronger distillation signal
distill_aggregation=layer_avg       # layer_avg: layer 출력을 평균 내고 비교, output_avg: 각 layer loss를 평균
