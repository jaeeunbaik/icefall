...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="contrastive",
        choices=["contrastive", "hybrid", "asr"],
        help="Training Type: 'contrastive' for contrastive learning only encoder, 'hybrid' for hybrid(contrastive + sd + asr), 'asr' for ctc loss",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=10.0,
        help="Beta for hybrid loss, how much contrastive loss is summed to total hybrid loss, distillation loss is adjusted by alpha",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="clean",
        choices=["clean", "noisy", "clean-noisy"],
...
def compute_loss(
    ...
    if params.learning_type == "contrastive":
        ...
        logging.debug(f"Pure contrastive loss: {total_loss:.4f}")
        
    elif params.learning_type == "hybrid":
        # Hybrid: ASR + distillation + contrastive
        total_loss = ctc_loss  # ASR loss (CTC only)
        
        # Add self-distillation loss with alpha weight
        if use_self_distillation and distillation_loss.item() > 0:
            total_loss = total_loss + params.alpha * distillation_loss
            
        # Add contrastive loss with beta weight
        if contrastive_loss.item() > 0:
            total_loss = total_loss + params.beta * contrastive_loss
            
        logging.debug(f"Hybrid loss: ctc={ctc_loss:.4f}, "
                     f"distill={distillation_loss:.4f}*{params.alpha}, "
                     f"contrastive={contrastive_loss:.4f}*{params.beta}, "
                     f"total={total_loss:.4f}")
        
    elif params.learning_type == "asr":
        # Pure ASR loss (CTC only) - no distillation
        total_loss = ctc_loss
        logging.debug(f"Pure ASR loss (CTC only): {total_loss:.4f}")
    ...
    info["contrastive_loss"] = contrastive_loss.detach().cpu().item()
...
def compute_contrastive_loss(
    ...
    Args:
        clean_features: Features from clean input (T, N, C)
        noisy_features: Features from noisy input (T, N, C)
        negative_pair: Type of negative pairs ("clean", "noisy", "clean-noisy")
        temperature: Temperature parameter for InfoNCE loss (lower = more focused on hard negatives)
        
    Returns:
        Contrastive loss tensor
    """
    device = clean_features.device
    T, N, C = clean_features.shape
    
    # Ensure clean and noisy have same shape
    assert clean_features.shape == noisy_features.shape, f"Shape mismatch: clean {clean_features.shape} vs noisy {noisy_features.shape}"
    
    # L2 normalize features
    clean_norm = F.normalize(clean_features, p=2, dim=-1)  # (T, N, C)
    noisy_norm = F.normalize(noisy_features, p=2, dim=-1)  # (T, N, C)
