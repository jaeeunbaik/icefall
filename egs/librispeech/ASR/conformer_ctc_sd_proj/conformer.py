...
class Conformer(Transformer):
    ...
    def __init__(
        ...
        learning_type: str = "asr",  # Memory optimization based on learning type
    ) -> None:
        # Store learning type for memory optimization
        self.learning_type = learning_type
        
        # Memory optimization: adjust num_decoder_layers based on learning_type
        if learning_type == "encoder-only":
            # Encoder-only mode doesn't need decoder at all
            num_decoder_layers = 0
            logging.info(f"Memory optimization: Disabled decoder for encoder-only mode")
        ...
        if learning_type == "encoder-only":
            ...
            if hasattr(self, 'linear'):
                self.linear = None
            logging.info(f"Memory optimization: Disabled CTC output layers for encoder-only mode")
    ...
    def get_intermediate_outputs(
        ...
        """
        # Memory optimization: Skip for ASR-only mode
        if self.learning_type == "asr" or not self.distill_layers:
            return []
        ...
        # Apply projection heads to selected layer outputs - Memory optimization
        projected_embeddings = []
        if self.use_proj_layer and self.distill_projection_heads and self.learning_type != "asr":
...
class ConformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        attn_output, att_maps = self.self_attn(
            src,
            src,
            src,
            pos_emb=pos_emb,
...
class ConformerEncoder(nn.TransformerEncoder):
    ...
    def forward(
        ...
        return output, layer_outputs, attention_maps
...
def compute_distillation_loss(
    ...
    Args:
        teacher_knowledge: Teacher model outputs (list of tensors for layers or single tensor)
    ...
    if loss_type == "mse":
        ...
        if prototype_manager is not None and layer_idx is not None:
            ...
            try:
                # For prototype-based KL, features need to match prototype dimensions
                # This is a fallback - ideally projection should happen before this function
...
def compute_multi_layer_distillation_loss(
    ...
    Args:
        teacher_knowledge: List of teacher model outputs for each layer
        student_knowledge: List of student model outputs for each layer  
        knowledge_lens: Length of valid frames for masking padding
    ...
    """
    device = teacher_knowledge[0].device
    
    # Convert layer_indices to 0-based indices (subtract 1 from each)
    # This handles the case where layer indices are provided as 1-based (e.g., 6,12,18)
    # but need to be used as 0-based for list indexing (e.g., 5,11,17)
    adjusted_layer_indices = [max(0, i - 1) for i in layer_indices]
    logging.debug(f"Adjusted layer indices from {layer_indices} to {adjusted_layer_indices}")
    ...
    if aggregation == "layer_avg":
        ...
        teacher_avg = torch.stack([teacher_knowledge[i] for i in adjusted_layer_indices], dim=0).mean(dim=0)
        student_avg = torch.stack([student_knowledge[i] for i in adjusted_layer_indices], dim=0).mean(dim=0)
        ...
        if layer_weights is None:
            layer_weights = [1.0] * len(adjusted_layer_indices)
        elif len(layer_weights) != len(adjusted_layer_indices):
            logging.warning(f"layer_weights length ({len(layer_weights)}) doesn't match adjusted_layer_indices length ({len(adjusted_layer_indices)}). Using equal weights.")
            layer_weights = [1.0] * len(adjusted_layer_indices)
        
        for idx, adjusted_layer_idx in enumerate(adjusted_layer_indices):
            if adjusted_layer_idx >= len(teacher_knowledge) or adjusted_layer_idx >= len(student_knowledge):
            ...
            layer_loss = compute_distillation_loss(
                teacher_knowledge=teacher_knowledge[adjusted_layer_idx],
                student_knowledge=student_knowledge[adjusted_layer_idx],
                knowledge_lens=knowledge_lens,
                loss_type=loss_type,
                temperature=temperature,
                prototype_manager=prototype_manager,
        ...
        if losses:
            ...
            # Auto-scaling with fixed multiplier to preserve relative differences
            loss_magnitude = loss.item()
