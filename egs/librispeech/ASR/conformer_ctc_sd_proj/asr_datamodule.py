    ...
    def train_dataloaders(
        ...
        if enable_self_distillation:
            ...
            if getattr(self.args, 'enable_clean_augmentation', False):
                ...
                cuts_musan = load_manifest("data/fbank/musan_cuts.jsonl.gz")
                clean_cut_transforms.append(
                CutMix(cuts=cuts_musan, p=self.args.clean_augmentation_prob, snr=(0, 10), preserve_id=True)
                )
                # Clean SpecAugment (much lighter parameters)
                clean_specaugment = SpecAugment(
                    time_warp_factor=0,  # No time warping for clean
                    num_frame_masks=self.args.clean_specaugment_time_mask_max_frames,
                    features_mask_size=self.args.clean_specaugment_freq_mask_max_bins,
                    num_feature_masks=2,
                    frames_mask_size=3
                    )
